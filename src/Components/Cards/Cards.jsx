...
const Cards = () => {
    ...
    const fetchVehicleData = async () => {
      try {
        ...
        const groupedVehicles = vehicleData.reduce((acc, vehicle) => {
          ...
          acc[vehicle.vehicle_id].numEmployees += 1;
          return acc;
        }, {});
  ...
  const filteredVehicles = vehicles.filter(vehicle =>
    vehicle.vehicle_id && vehicle.vehicle_id.toString().includes(searchTerm)
  );
        ...
        <div className='cards--cards'>
          {filteredVehicles.map(vehicle => (
            <div
              ...
              className={`card ${selectedVehicle && selectedVehicle.vehicle_id === vehicle.vehicle_id ? 'active-card' : ''}`}
              onClick={() => handleSelectVehicle(vehicle)}
            >
              <h3 className='heading3'>Vehicle ID: {vehicle.vehicle_id}</h3>
              <p>Employees: {vehicle.numEmployees}</p>
            </div>
          ))}
        </div>
